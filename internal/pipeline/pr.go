...
type PullRequestCreator interface {
	Create(ctx context.Context, repoDir, title, bodyFile string) (note string, url string, err error)
}

type GHClient struct {
	lookupPath func(string) (string, error)
	runner     func(ctx context.Context, dir string, name string, args ...string) (string, error)
}
...
func (g *GHClient) Create(ctx context.Context, repoDir, title, bodyFile string) (string, string, error) {
	lookup := g.lookupPath
	if lookup == nil {
	...
	out, err := runner(ctx, repoDir, "gh", "pr", "create", "--title", title, "--body-file", bodyFile)
	if err != nil {
		return "", "", err
	}
