...
The existing event system in `pipeline.emit()` already fires typed events for every state transition, phase attempt, review result, and summary. The Pi RPC adapter (`internal/agent/pi.go`) also produces internal events through its `Events()` channel for session lifecycle, model selection, RPC responses, and crash recovery. Both event sources are currently consumed by the TUI and then discarded. Writing them to disk as JSONL is a small, self-contained change that unlocks debugging, benchmarking, and cross-run comparison.

The investigation of the Pi adapter's session management bugs (documented in `INVESTIGATION.md`) proved that pipeline-level events alone are insufficient for diagnosing failures. The most insidious bugs — silent model failures, stale event accumulation, `agent_end` firing per-turn — are invisible without seeing what happens inside the adapter. Tracing both pipeline and adapter events makes future debugging deterministic instead of requiring ad-hoc `NELSONCTL_DEBUG` sessions.
...
- Capture Pi adapter internals in traces: session creation/switching, model selection success/failure, RPC event summaries (stop reasons, event draining), and crash recovery. These events flow through the agent's `Events()` channel into the trace writer.
...
- Not capturing token-level streaming content or thinking tokens from the agent's LLM. Session lifecycle, model selection, and RPC event summaries are in scope — raw token streams are not.
- Not capturing tool call arguments or results from the agent's internal tool use (file reads, edits, bash commands). The trace records that the agent was invoked and what it produced, not the step-by-step reasoning inside the agent session.
