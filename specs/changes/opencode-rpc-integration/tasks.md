...
- [ ] Implement `internal/agent/ocsse.go` — SSE client that reads `GET /global/event`, parses event frames, coalesces text deltas onto a configurable render tick, and forwards typed events to the `Events() <-chan Event` channel. Runs on a separate goroutine from blocking message calls. Supports `SSE Event Consumption` and `Event Coalescing`.
